...
class FairRobust(Sampler):
    ...
    Attributes:
        ...
        algorithm: A string indicating the used classification algorithm
                   among lr and svm
    ...
    def __init__(self, model, x_tensor, y_tensor, z_tensor, target_fairness, algorithm, parameters,
    ...
    def adjust_lambda(self, logit):
        ...
        if self.fairness_type == 'eqopp':
            ...
            if self.algorithm == 'lr':
                eo_loss = nn.BCELoss(reduction='none')((F.tanh(logit.squeeze()) + 1) / 2, (self.y_data.squeeze() + 1) / 2)
            elif self.algorithm == 'svm':
                eo_loss = torch.clamp(1 - self.y_data.squeeze() * logit.squeeze(), min=0)
            ...
            for tmp_yz in self.yz_tuple:
                yhat_yz[tmp_yz] = float(torch.sum(eo_loss[self.clean_yz_index[tmp_yz]])) / (
                            self.clean_yz_len[tmp_yz] + 1)
            ...
            if self.algorithm == 'lr':
                dp_loss = nn.BCELoss(reduction='none')((F.tanh(logit.squeeze()) + 1) / 2,
                                                       ones_tensor.squeeze())  # Note that ones tensor puts as the true label
            elif self.algorithm == 'svm':
                dp_loss = torch.clamp(1 - ones_tensor.squeeze() * logit.squeeze(), min=0)
    ...
    def select_fair_robust_sample(self):
        ...
        if self.algorithm == 'lr':
            loss = nn.BCELoss(reduction='none')((F.tanh(logit.squeeze()) + 1) / 2, (self.y_data.squeeze() + 1) / 2)
        elif self.algorithm == 'svm':
            loss = torch.clamp(1 - self.y_data.squeeze() * logit.squeeze(), min=0)
